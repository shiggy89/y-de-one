...
export default function Access({ variant }: { variant?: "modern-ballet" }) {
      ...
      <div className="inner">
        ...
        <div className={styles.trialCard}>
          ...
          <div className={styles.accessCardRight}>
            <h3>
              📍{isModernBallet ? "Y-de-ONE モダンバレエ教室" : "Y-de-ONE バレエ教室"}<br />
              　〒169-0075<br />
              　東京都新宿区高田馬場3-36-6<br />
              　兼子ビル2階
            </h3>
            <div className={styles.accessFlexGrid}>
              {[
                ["#takadanobaba", "高田馬場駅からの"],
                ["#higashinakano", "東中野駅からの"],
                ["#ochiai", "落合駅からの"],
                ["#shinjuku", "新宿駅からの"],
              ].map(([hash, label]) => (
                <div key={hash} className={`${styles.accessBox}${isModernBallet ? ` ${styles.accessBoxModernBallet}` : ""}`}>
                  <a href={`/access${hash}`}>{label}<br />アクセスを見る <i className="fa-solid fa-arrow-up-right-from-square"></i></a>
                </div>
              ))}
            </div>
          </div>
        </div>
        {!isModernBallet && <SectionCtaButton />}
      </div>
    </section>
  );
}
